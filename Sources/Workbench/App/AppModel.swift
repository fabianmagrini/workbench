...
final class AppModel {
    ...
    func runSelectedTask(context: ModelContext) {
        ...
        runningJobs[task.id] = Swift.Task { [weak self] in
            ...
            do {
                ...
                context.insert(LogEntry(level: .error, message: error.localizedDescription, session: session))
            }
            runningJobs[task.id] = nil
            try? context.save()
        }
    ...
    func cancelSelectedTask(context: ModelContext) {
        ...
        if let session = task.sessions.last {
            ...
            context.insert(LogEntry(level: .warning, message: "Execution cancelled", session: session))
        }
